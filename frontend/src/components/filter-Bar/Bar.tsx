...
import { fetchAllNotes, setShowFilterBar } from '../../features/routine/RoutineSlice';
...
const FilterBar = ({ filterBarRef, setShowFullAddForm }: FilterBarProps) => {
    ...
    const [customLabels, loadingCustomLabels] = useFetchData<ICustomLabel>('GET', `${config.server.url}/routine/${user._id}`, 'customLabels');
    ...
    const { oldestTaskDate } = useAppSelector((store) => store.routine);
    const { isFilterBarShown } = useAppSelector((store) => store.routine);
    const dispatch = useAppDispatch();
    const [wasReset, setWasReset] = useState(false);
    const [searchQuery, setSearchQuery] = useState<string>('');
    const [sortType, setSortType] = useState<SortOptions>(SortOptions.NEWEST);
    const [startDate, setStartDate] = useState<number>(oldestTaskDate);
    ...
    const filterData = { sortType, startDate, endDate, type, category, importanceMin, importanceMax, showAnyCategory, wasReset, oldestTaskDate, showAnyType, allTypes, allCategories };
    ...
    return (
        ...
        >
            <div className={`fl justify-between flex-wrap transition-all duration-300 ${isFilterBarShown ? 'delay-300' : 'opacity-0 invisible'}`}>
                ...
                <FilterSearchInput
                    inputClassName="filter-search-input rounded-lg py-3 h-full min-w-[12rem] pr-8  hover:text-white focus-within:text-white focus-within:bg-green-600 hover:bg-green-600 focus-within:placeholder:text-white hover:placeholder:text-white"
            ...
            <span
                ...
                className={`text-4xl absolute -bottom-7 text-green-600 z-50 cursor-pointer transition-all duration-300 block right-4 ${isFilterBarShown ? 'rotate-180' : ''} hover:text-green-700`}
            >
                <IoIosArrowDown />
            </span>
            <span
                onClick={resetFilters}
                className={`text-lg absolute -bottom-[22px] px-[6px] py-[1px] bg-green-500 text-white rounded-lg z-50 cursor-pointer transition-all duration-300 block right-16  hover:bg-green-600`}
            >
                Reset
            </span>
        </div>
