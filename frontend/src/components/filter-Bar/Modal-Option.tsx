...
const FilterModalOption = ({ text, onchange, checkedAtStart, showCheckmark = true, canToggle = true, forceCheck, refToClick }: FilterModalOptionProps) => {
    ...
    return (
        <label
            ...
            className={`block sm:text-xl text-lg filter-modal-option relative px-10 sm:py-2 py-1 border-solid border-2 w-full border-green-100 rounded-md sm:mb-3 mb-2 cursor-pointer transition-all ${
                checked ? 'bg-green-600 border-green-600 text-white' : ''
            } hover:bg-green-500 hover:text-white duration-300 hover:shadow-md hover:border-green-500`}
        >
            <input
