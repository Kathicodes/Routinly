    ...
    if (otherNote) {
        return (
            <Link to={`/update/${otherNote._id}`} className={`other-note-btn cursor-pointer basis-[49%] w-1/2 flex-grow-0 flex-shrink-0 ${isPrev ? 'text-left' : 'text-right'}`}>
                <h5 className={`font-semibold fl text-sm text-green-800 uppercase transition-colors mb-1 ${isPrev ? 'justify-start' : 'justify-end'}  -tracking-normal`}>
                ...
                <h4 className={`sm:text-lg text-base ${isPrev ? 'mr-auto' : 'ml-auto'} transition-all duration-300 text-green-600 whitespace-nowrap overflow-hidden text-ellipsis max-w-[95%]`}>
                    {otherNote.title}
                </h4>
            </Link>
        );
