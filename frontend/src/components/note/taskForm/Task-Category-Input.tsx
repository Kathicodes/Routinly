...
import { setError, setSuccess } from '../../../features/routine/RoutineSlice';
...
interface TaskLabelInputProps {
    ...
    label: ICustomLabel[];
    disabled?: boolean;
    setTaskColor: React.Dispatch<React.SetStateAction<string>>;
}

const TaskCategoryInput = ({ label, setLabel, setTaskColor, disabled }: TaskLabelInputProps) => {
    const { user } = useAppSelector((store) => store.user);
    const [fetchLabels, setFetchLabels] = useState(false);
    const [customLabels] = useFetchData<ICustomLabel>('GET', `${config.server.url}/routine/${user._id}`, 'customLabels', fetchLabels);
    ...
    const addNewLabel = async () => {
        ...
        if (newLabel.trim() === '') {
            dispatch(setError(`Task ${labelName} cannot be empty.`));
        } else if (!isNew) {
            dispatch(setError(`Task ${labelName} "${newLabel}" already exists.`));
        } else {
            try {
                const response = await axios({
                    method: 'POST',
                    url: `${config.server.url}/routine/create`,
                    data: {
                        labelName: newLabel,
                        color: color,
                ...
                if (response.status === 201) {
                    ...
                    dispatch(setError(`Unable to create task ${labelName}.`));
                }
            } catch (error: any) {
    ...
    const deleteLabel = async (labelToDelete: ICustomLabel) => {
        ...
        try {
            const response = await axios({
                method: 'DELETE',
                url: `${config.server.url}/routine/${labelToDelete._id}`
            });

            if (response.status === 200) {
                dispatch(setSuccess(`Task ${labelName} "${labelToDelete.labelName}" has been deleted.`));
                setFetchLabels(!fetchLabels);
                ...
                dispatch(setError(`Unable to delete task ${labelName}.`));
            }
        } catch (error: any) {
            dispatch(setError(error.message));
        }
    ...
    return (
        <div className="relative categories-form">
            ...
            <label
                style={{ color: color }}
                htmlFor={'taskCustomCategoryColorInput'}
            ...
            <input
                type="color"
                id={'taskCustomCategoryColorInput'}
                className="hidden"
                disabled={disabled === undefined ? false : disabled}
                value={color}
                onChange={(e) => {
            ...
            <ul
                className={`categories-list rounded-b-xl shadow-xl overflow-y-auto h-auto max-h-0 opacity-0 sm:left-1/2 left-0 overflow-hidden sm:-translate-x-1/2 sm:w-full w-[70vw] transition-all duration-300 absolute bg-green-600 bottom-0 translate-y-full z-[200] text-white`}
            >
                    ...
                    return (
                        <li
                            className={`relative block whitespace-nowrap overflow-hidden text-ellipsis px-8 py-2 text-lg tracking-wide transition-all cursor-pointer ${
                                label.map((l) => l._id).includes(customLabel._id) ? '!bg-green-500 font-semibold' : ''
                            } hover:bg-green-700`}
                            ...
                            {customLabel.color && (
                                <button
                                    ...
                                    onClick={(e) => {
                                        if (disabled) return;
                                        e.stopPropagation();
                                        setTaskColor(customLabel.color!);
                                    }}
                                    className="absolute z-10 left-2 top-1/2 -translate-y-1/2 cursor-pointer block w-8 h-8 rounded-full border-solid border-2 shadow-sm border-white transition-all hover:border-green-200 hover:shadow-md"
                                ></button>
                            )}
                            <button
                                type="button"
                                ...
                                className="absolute right-2 top-1/2 rounded-md -translate-y-1/2 text-xl p-[5px] transition-colors bg-green-800 hover:bg-green-900"
                            >
            ...
            {focused && (
                <button
                    ...
                    type="button"
                    className="absolute sm:right-0 -right-2 top-1/2 py-2 px-2 -translate-y-1/2 text-2xl hover:text-green-500 text-green-400 transition-colors"
                    onClick={addNewLabel}
                >
                    <AiOutlineArrowRight />
                </button>
