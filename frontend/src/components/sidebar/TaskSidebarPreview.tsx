    ...
    return (
        <div
            ...
            className={`sm:pt-2 pt-1 sm:pb-1 pb-[2px] px-4 ${
                currentNoteOpenedId === note._id ? '!bg-green-700' : 'bg-green-800'
            } my-[2px] transition-all duration-200 hover:bg-green-900 text-left w-full ${isInitialNote ? '' : 'cursor-pointer'}`}
        >
            <h4 className="text-lg whitespace-nowrap overflow-hidden text-ellipsis">{note.title}</h4>
            <div className="flex-between">
                <h5 className="text-sm text-green-500">{new Date(note.startDate).toDateString()}</h5>
                <div>
                    {note.isStarred && <NoteInfo className="!text-base !py-[1px] !px-[6px] mr-1" text={<AiFillStar className="text-[1.3rem] inline-block pb-1" />} color="#007c36" />}
                    <NoteInfo text={note.type ? note.type.labelName : defaultNoteTypes[0].labelName} color="#007c36" className="!text-base mr-0 !py-[1px] !px-[6px]" />
                </div>
            </div>
        </div>
    );
