...
const Sidebar = ({ sidebarRef, topRef }: SidebarProps) => {
    ...
    return (
        <div
            ...
            } xs:w-[28rem] w-[100%] transition-all z-[2000] ease-in-out bg-green-900 overflow-x-hidden text-white text-left text-xl`}
        >
            <div className="fl sm:h-[65px] h-[50px] sticky top-0 left-0 bg-green-900 z-[200]">
                <button onClick={() => dispatch(setShowSidebar(!isSidebarShown))} className={`text-4xl transition-colors lg:hidden absolute left-4 top-1/2 -translate-y-1/2 hover:text-green-100`}>
                    <IoMdMenu />
                </button>
                <Link
                    to={'/'}
                    ...
                    className="lg:text-3xl text-2xl lg:ml-0 ml-12 w-[28rem] whitespace-nowrap overflow-hidden text-ellipsis font-semibold px-4 block break-all bg-green-900 hover:text-green-100 transition-colors"
                >
                    {user.name.split(' ')[0]}'s Routine
                </Link>
            </div>
            <div className="relative flex-between bg-green-800">
                <FilterSearchInput
                    searchQuery={searchQuery}
                    isSidebar={true}
                    labelClassName="text-green-800 left-6 text-2xl"
                    deleteClassName="text-green-800 !right-6"
                    setSearchQuery={setSearchQuery}
                    inputClassName="mx-4 text-green-800 shadow-lg my-3 rounded-2xl pl-9 pr-9 py-1 border-solid transition-all border-green-500 focus:border-b-[3px] hover:bg-green-50"
                />
            </div>
            <div>
                <Link
                    ...
                    className="text-xl new-note py-5 px-4 bg-green-500 font-semibold flex items-center cursor-pointer duration-300 transition-colors hover:bg-green-600"
                >
                ...
                <button
                    ...
                    className={`text-lg starred-btn py-5 px-4 w-full font-semibold flex items-center cursor-pointer duration-300 transition-colors hover:bg-green-800 ${
                        starred ? '!bg-green-600' : 'bg-green-700'
                    }`}
                >
