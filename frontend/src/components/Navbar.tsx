    ...
    return (
        <div
            className={`flex sticky top-0 left-0 right-0 z-[999] shadow-lg items-center transition-all duration-500 justify-between ${
                isSidebarShown && width > 1024 ? 'small-padding-x' : 'padding-x'
            } py-4 text-white bg-green-600 sm:h-[65px] h-[50px]`}
        >
            <button onClick={() => dispatch(setShowSidebar(!isSidebarShown))} className={`text-4xl transition-colors absolute left-4 top-1/2 -translate-y-1/2 hover:text-green-100`}>
                <IoMdMenu />
            </button>
            <Link
                to={'/'}
                onClick={() => topRef && topRef.current.scrollIntoView({ behavior: 'smooth' })}
                className="md:text-3xl text-2xl font-bold cursor-pointer transition-colors hover:text-green-100"
            >
                <span className="sm:block hidden md:ml-4">Routinly</span>
            ...
            <div className="flex items-center">
                ...
                <button onClick={() => Logout()} className="sm:text-2xl text-xl cursor-pointer transition-all bg-green-500 sm:px-4 px-3 py-1 rounded-md hover:bg-green-600 hover:shadow-sm">
                    Logout
                </button>
            </div>
        </div>
